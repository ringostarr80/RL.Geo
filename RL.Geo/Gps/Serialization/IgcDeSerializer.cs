    ...
    public class IgcDeSerializer : IGpsFileDeSerializer
    {
        ...
        private bool ParseFix(string line, TrackSegment trackSegment, DateTime date)
        {
            ...
            if (match.Success)
            {
                ...
                var fix = new Fix(cood.Latitude, cood.Longitude, double.Parse(gpsAlt, CultureInfo.InvariantCulture), date.AddHours(int.Parse(h)).AddMinutes(int.Parse(m)).AddSeconds(int.Parse(s)));
