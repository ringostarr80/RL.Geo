    ...
    public class Coordinate : SpatialObject, IPosition
    {
        ...
        public static bool TryParse(string coordinate, out Coordinate result)
        {
            ...
            if (match.Success)
            {
                ...
                double temp;
                if (double.TryParse(match.Groups["Min1"].Value, NumberStyles.Float, CultureInfo.InvariantCulture, out temp))
                    deg1 = deg1 + temp / 60;

                if (double.TryParse(match.Groups["Min2"].Value, NumberStyles.Float, CultureInfo.InvariantCulture, out temp))
                    deg2 = deg2 + temp / 60;

                if (double.TryParse(match.Groups["Sec1"].Value, NumberStyles.Float, CultureInfo.InvariantCulture, out temp))
                    deg1 = deg1 + temp / 3600;

                if (double.TryParse(match.Groups["Sec2"].Value, NumberStyles.Float, CultureInfo.InvariantCulture, out temp))
                    deg2 = deg2 + temp / 3600;
